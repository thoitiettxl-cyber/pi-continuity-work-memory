...
import { ContinuityService } from "./application/continuity-service.js";
...
import { GitFingerprintService, repositoryIdForRoot, workspaceId, } from "./infrastructure/git-fingerprint.js";
...
import { CONTINUITY_ENTRY_TYPE, branchContext, memorySource, sessionFileKey, sessionKey, } from "./interface/session-adapter.js";
...
export default function extension(pi) {
    ...
    let authorityCompromisedReason;
    let generationToken = "0";
    const manualPipelineControllers = new Set();
    const commandRunner = {
    ...
    const requireSafeAuthority = () => {
        if (authorityCompromisedReason)
            throw new Error(`Safe authority is unavailable: ${authorityCompromisedReason}`);
    };
    const refreshTuiStatus = async (ctx) => {
        if (!continuity) {
            setTuiStatus(ctx, "unavailable");
            return;
        }
    ...
    const runMemoryPipeline = async (ctx, signal, schedulerGeneration) => {
        if (!memory)
            return;
        ...
        if (result.status === "failed")
            safeNotify(ctx, `Memory pipeline failed: ${result.reason}`, "warning");
    };
    async function startSession(event, ctx) {
        context = ctx;
    ...
    pi.on("session_compact", async (_event, ctx) => {
        context = ctx;
        if (continuity)
            continuity.reconstructBranch(branchContext(ctx));
        appendState();
    });
    ...
    pi.on("user_bash", async (event, ctx) => {
        ...
        if (!continuityStore?.getTrackedCall(toolCallId))
            return;
    ...
    pi.registerTool({
        ...
        async execute(_id, params, _signal, _update, ctx) {
            const services = requireServices();
            const state = services.continuity.update(params, branchContext(ctx));
            appendState();
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!record)
                throw new Error("Memory not found or not visible in this scope");
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            scope: StringEnum(["repository", "work-item", "session"]),
