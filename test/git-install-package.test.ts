...
test("package declares the clean Git-install build and validation contract", () => {
	...
	assert.equal(manifest.scripts?.["validate:git-install"], "bun scripts/validate-git-install.mjs");
	assert.match(manifest.scripts?.validate ?? "", /bun scripts\/validate-git-install\.mjs/);
	assert.equal(manifest.scripts?.test, "bun test --max-concurrency=1 --timeout=30000 test");
...
test("package engines, peer range, and proof identity stay aligned for RC8 install validation", () => {
	const manifest = readJson("package.json") as {
		version?: string;
		engines?: { bun?: string };
	...
	assert.equal(manifest.peerDependencies?.["@earendil-works/pi-coding-agent"], ">=0.87.0 <0.88.0");
	assert.equal(manifest.peerDependencies?.["@earendil-works/pi-ai"], ">=0.87.0 <0.88.0");
	...
	assert.match(results.status ?? "", /RC8/);
	assert.match(acceptance, /1\.0\.0-rc\.8/);
	assert.match(acceptance, />=0\.87\.0 <0\.88\.0/);
});

test("GitHub Actions CI runs bun typecheck and test without a Node setup", () => {
