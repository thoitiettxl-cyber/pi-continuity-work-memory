...
import { buildCheckpointHashes, verifyCheckpointChain } from "../domain/checkpoint-chain.js";
import { CONTINUITY_SCHEMA_VERSION, cloneWorkState, emptyWorkState, } from "../domain/types.js";
import { ContinuityStore } from "../infrastructure/continuity-store.js";
import { GitFingerprintService, } from "../infrastructure/git-fingerprint.js";
...
function latestEmbedded(branch) {
    for (let index = branch.embeddedStates.length - 1; index >= 0; index -= 1) {
        const embedded = branch.embeddedStates[index];
        if (embedded?.schemaVersion === CONTINUITY_SCHEMA_VERSION)
            return embedded;
    }
    return undefined;
}
function payloadState(record) {
...
export class ContinuityService {
    ...
    started = false;
    constructor(identity, cwd, store, fingerprints, commandRunner) {
    ...
    update(patch, branch) {
        ...
        this.state = this.store.mutateState(this.identity.sessionKey, branch.currentNodeId, this.state, (state) => {
            if (patch.goal !== undefined)
                state.goal = patch.goal.trim().slice(0, 16_000);
            if (patch.workItemId !== undefined)
                state.workItemId = patch.workItemId.trim().slice(0, 500) || "default";
            if (patch.plan !== undefined)
                state.plan = normalizePlan(patch.plan);
            if (patch.currentStepId !== undefined)
                state.currentStepId = patch.currentStepId?.trim().slice(0, 200) || null;
            if (patch.nextActions !== undefined)
                state.nextActions = boundedStrings(patch.nextActions);
            if (patch.completedWork !== undefined)
                state.completedWork = boundedStrings(patch.completedWork);
            if (patch.decisions !== undefined)
                state.decisions = boundedStrings(patch.decisions);
            if (patch.blockers !== undefined)
                state.blockers = boundedStrings(patch.blockers);
            if (patch.constraints !== undefined)
                state.constraints = boundedStrings(patch.constraints);
        });
        return this.currentState();
    }
    async observeToolCall(observation) {
        this.requireStarted();
        const classification = classifyTool(observation.toolName, observation.input);
        if (classification !== "mutation" && classification !== "validation")
            return;
    ...
    async observeToolResult(observation) {
        this.requireStarted();
        const trackedBefore = this.store.getTrackedCall(observation.toolCallId);
        if (!trackedBefore)
            return undefined;
        const tracked = this.store.resolveTrackedCall(observation.toolCallId, observation.isError, sha256(redactSecrets(observation.contentText)));
        if (!tracked)
            return undefined;
        ...
        if (observation.isError || !this.identity.trusted || !tracked.command || !tracked.preFingerprint)
            return undefined;
    ...
    async validate(command, branch, signal) {
        this.requireStarted();
        if (!this.identity.trusted)
            throw new Error("Project is untrusted; validation cannot collect Git authority");
        if (!isExecutableValidationCommand(command))
            throw new Error("Validation command is not on the executable allow-list or contains shell operators");
    ...
    recordEvidence(evidence, branch) {
    ...
    async createCheckpoint(branch, signal) {
        this.requireStarted();
        if (!this.identity.trusted)
            throw new Error("Project is untrusted; safe checkpoints require a complete Git fingerprint");
        ...
        if (!evidence)
            throw new Error("No successful executable validation exists after the latest mutation");
    ...
    async status(branch, signal) {
        this.requireStarted();
        const result = (health, checkpointId, authority, reason) => ({ health, checkpointId, authority, reason, lineage: this.lineage(), state: this.currentState() });
        if (!this.identity.trusted) {
        ...
        if (!checkpointId)
            return result("degraded", null, "none", "no checkpoint");
    ...
    recover(branch, requestedCheckpointId) {
        this.requireStarted();
        ...
        if (requestedCheckpointId) {
            ...
            if (checkpoint && checkpoint.repositoryId === this.identity.repositoryId)
                recovered = payloadState(checkpoint);
        }
        if (!recovered)
            recovered = latestEmbedded(branch)?.state;
        if (!recovered)
            recovered = this.store.findNearestState(this.identity.sessionKey, branch.nodeIds);
        if (!recovered)
            throw new Error("No continuity state is available for recovery");
        this.state = this.store.saveState(this.identity.sessionKey, branch.currentNodeId, cloneWorkState(recovered));
        return this.currentState();
    }
    embeddedState(checkpoint) {
        return {
    ...
    requireStarted() {
        if (!this.started)
            throw new Error("Continuity service has not started");
    }
}
