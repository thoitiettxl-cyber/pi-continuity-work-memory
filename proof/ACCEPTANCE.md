...
## Current RC8 Pi 0.87 runtime alignment

`PARTIAL` — package identity is **1.0.0-rc.8**. Host contract is Pi **`>=0.87.0 <0.88.0`**. Development commands use Bun `1.4.2`; the `node` binary is not required. `before_agent_start` still injects Continuity, managed workflow, and learning memory through `systemPromptOptions.sections` and must not return `{ systemPrompt }` or set `forceSystemPrompt`. Provider-bound memory source follows Pi 0.87 canonical session context, including `context_edit` omissions and replacements. Observed on this host: strict typecheck PASS and `bun test` **287/287**. Install, Git-install, release, and premerge were not run. The RC7 Pi 0.86 section remains historical.

Historical RC7 observation, not a current PASS: Ubuntu **24.04.5 LTS** aarch64, Node **v22.19.0**, Bun **1.4.2**, live Pi **0.86.0**. Current RC8 gates are recorded in `proof/RESULTS.json` only after they are run.
...
| Query-conditioned learning memory | `test/memory.test.ts`, `test/memory-context-budget.test.ts`, `test/memory-context-evaluation.test.ts`, `test/extension-mode.test.ts`, `test/managed-workflow-extension.test.ts`, `test/provider-source-sanitization.test.ts`, `test/sqlite-migrations.test.ts`, `src/domain/memory-context-budget.ts` | `before_agent_start` conditions recall on the current prompt and passes `ctx.model?.contextWindow` in tui, rpc, json, and print without non-interactive UI access; search scans visible published records by token overlap without a 500-candidate cutoff and does not require a contiguous substring; estimated budgeting (`ceil(chars/4)` labeled estimate) uses fallback 16384→8192 characters, 32768→16384, and 128000+→64000, omits the whole block below wrapper+64, and UTF-16-safe truncation keeps complete delimiters; rendering preserves baselines, matched atoms (including citation-tail matches), and the closing authority delimiter when the budget can hold them; search-throw still emits baselines; kinds default to `fact`; incremental extract skips below threshold unless forced; batch and concurrently staged exact-content duplicates stay singular; cursor-write failure rolls back publication; RC2 stores traverse to schema v3 with legacy `kind=fact`. | Covered by the listed tests; not a substitute for `npm run validate` |
...
- Version support: `PASS` only when the actual binary satisfies
  `>=0.87.0 <0.88.0`; unsupported or malformed versions fail with the range.
