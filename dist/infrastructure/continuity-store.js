...
`);
    ...
    registerSession(identity, now = Date.now()) {
        this.db.transaction(() => {
...
ON CONFLICT(session_key) DO UPDATE SET
  ...
  updated_at = excluded.updated_at`).run(identity.sessionKey, identity.sessionId, identity.sessionFileKey, identity.parentSessionKey, identity.repositoryId, identity.trusted ? 1 : 0, now, now);
        });
    }
    mutateState(sessionKey, nodeId, inherited, mutator, now = Date.now()) {
...
WHERE session_key = ? AND node_id IN (${placeholders(branchNodeIds.length)})`).all(sessionKey, ...branchNodeIds);
    ...
    beginTrackedCall(input) {
        ...
        return this.db.transaction(() => {
            const row = this.db.prepare("SELECT state_json, node_id, updated_at FROM branch_states WHERE session_key = ? AND node_id = ?")
                .get(input.sessionKey, input.nodeId);
...
ON CONFLICT(tool_call_id) DO NOTHING`).run(input.toolCallId, input.sessionKey, input.nodeId, sequence, input.toolName, input.kind, input.inputDigest, input.command, input.preFingerprint, now);
            this.db.prepare(`INSERT INTO branch_states(session_key, node_id, state_json, revision, updated_at)
VALUES (?, ?, ?, 1, ?)
ON CONFLICT(session_key, node_id) DO UPDATE SET
  state_json = excluded.state_json,
  ...
  updated_at = excluded.updated_at`).run(input.sessionKey, input.nodeId, JSON.stringify(state), now);
    ...
    resolveTrackedCall(toolCallId, isError, resultDigest, now = Date.now()) {
        return this.db.transaction(() => {
            const row = this.db.prepare("SELECT * FROM pending_mutations WHERE tool_call_id = ?").get(toolCallId);
            if (!row)
                return undefined;
...
WHERE tool_call_id = ? AND status = 'pending'`).run(isError ? 1 : 0, resultDigest, now, toolCallId);
    ...
    markPendingUncertain(sessionKey, branchNodeIds, now = Date.now()) {
        if (branchNodeIds.length === 0)
            return [];
        return this.db.transaction(() => {
...
WHERE session_key = ? AND node_id IN (${placeholders(branchNodeIds.length)}) AND status = 'pending'`)
    ...
    pendingForBranch(sessionKey, branchNodeIds) {
        if (branchNodeIds.length === 0)
            return [];
        const rows = this.db.prepare(`SELECT * FROM pending_mutations
WHERE session_key = ? AND node_id IN (${placeholders(branchNodeIds.length)}) AND status = 'pending'
ORDER BY sequence`).all(sessionKey, ...branchNodeIds);
        return rows.map((row) => this.rowToPending(row));
    }
    getTrackedCall(toolCallId) {
        const row = this.db.prepare("SELECT * FROM pending_mutations WHERE tool_call_id = ?").get(toolCallId);
    ...
    recordValidation(sessionKey, nodeId, evidence) {
        this.db.transaction(() => {
...
ON CONFLICT(id) DO NOTHING`).run(evidence.id, sessionKey, nodeId, evidence.command, evidence.exitCode, evidence.startedAt, evidence.finishedAt, evidence.mutationSequence, evidence.repositoryFingerprint, evidence.outputDigest, evidence.provider);
        });
    }
    getValidation(id) {
        const row = this.db.prepare("SELECT * FROM validation_evidence WHERE id = ?").get(id);
        return row ? this.rowToEvidence(row) : undefined;
    }
    latestValidation(sessionKey, branchNodeIds, mutationSequence) {
        if (branchNodeIds.length === 0)
            return undefined;
...
ORDER BY finished_at DESC LIMIT 1`).get(sessionKey, ...branchNodeIds, mutationSequence);
        return row ? this.rowToEvidence(row) : undefined;
    }
    rowToEvidence(row) {
        return {
...
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(record.id, record.sessionKey, record.sessionId, record.sessionFileKey, record.repositoryId, record.parentId, record.parentHash, record.payloadJson, record.payloadHash, record.chainHash, record.repositoryFingerprint, record.validationEvidenceId, record.mutationSequence, record.status, record.createdAt);
        });
    }
    getCheckpoint(id) {
  ...
  id, source_session_key, source_session_file, target_entry_id, position, created_at
    ...
    consumeForkIntent(previousSessionFile, childSessionKey) {
...
ORDER BY created_at DESC LIMIT 1`).get(previousSessionFileKey);
            if (!row)
                return undefined;
