...
export interface CommandRunner {
	run(command: string, args: string[], options: { cwd: string; signal?: AbortSignal | undefined; timeout?: number }): Promise<CommandResult>;
}
...
function safeRepositoryPath(root: string, candidate: string): string {
	if (!candidate || isAbsolute(candidate) || candidate.includes("\0")) throw new Error(`Unsafe Git path: ${candidate}`);
...
async function hashUntracked(root: string, list: string): Promise<string> {
	const digest = createHash("sha256");
...
export class GitFingerprintService {
	...
	async repositoryRoot(cwd: string, trusted: boolean, signal?: AbortSignal): Promise<string> {
		if (!trusted) throw new RepositoryUnavailableError("Project is untrusted; Git access is disabled");
		const output = await requireSuccess(this.runner, cwd, ["rev-parse", "--show-toplevel"], signal);
		return realpath(output.trim());
	}

	async capture(cwd: string, trusted: boolean, signal?: AbortSignal): Promise<RepositoryFingerprint> {
		if (!trusted) throw new RepositoryUnavailableError("Project is untrusted; Git access is disabled");
		...
		const fingerprint: Omit<RepositoryFingerprint, "combined"> = {
			version: 1,
			repositoryRoot,
	...
	async captureStable(cwd: string, trusted: boolean, signal?: AbortSignal): Promise<RepositoryFingerprint> {
...
export function repositoryIdForRoot(root: string): string {
