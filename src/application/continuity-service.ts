...
const SESSION_OBJECTIVE_POLICY: Record<SessionObjectivePolicyKind, string> = {
	"bound-active": [
		...
		"When the bound plan's in-scope outcome is implemented, required proof is recorded, Result is no longer pending, and no remaining authorized in-scope delivery is listed, set Status to Ready for completion and call continuity_finalize_work in this turn without waiting for a second user request. Remaining in-scope commit, push, deploy, or other named delivery keeps the plan active.",
