...
- `/skill:contract-first` — coordinate independently evolving consumers and
  providers through one authoritative contract before implementation drift.
...
`improve-harness` are explicit-only. The other seven may be loaded automatically
...
The skills use no executable helper or runtime dependency. Their three
